...
const CircleComponent = ({ activeCircle, width, height, page }) => {console.log(page,"PAGE")
    const svgRef = useRef();
    ...
    useEffect(() => {
        const initialCircles = Array.from({ length: 4 }, (_, i) => ({
          ...
          imageUrl: page === 'Product' ? `./LandingPage_${i + 1}.png`: `./${page}_${i+1}.png`,
        }));
    ...
    const calculateUpdatedPositions = (activeCircleNum) => {
        ...
        return circles.map(circle => {
            if (circle.id === activeCircleNum) {
                ...
                let adjustedId = circle.id > activeCircleNum ? circle.id - 1 : circle.id;
    ...
    useEffect(() => {
        ...
        const updatedCircles = activeCircle >= 1 && activeCircle <= 4
