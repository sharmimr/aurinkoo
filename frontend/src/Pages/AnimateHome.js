...
const N = 40; // Number of vertices per line
const AnimatedLines = ({ numberOfLines, travelDistance = 6 }) => {
  const linesData = useMemo(() => {
    ...
    return new Array(numberOfLines).fill().map((_, lineIndex) => {
      ...
      for (let i = 0; i < N; i++) {
        color.setHSL(0.16 - 0.16 * (lineIndex / numberOfLines), 1, 0.5); // Yellow to red
...
const SamplePageTwo = ({ enableZoom = false, travelDistance = 30 }) => {
  return (
    <div className="row align-items-center position-relative">
      ...
      <div
        className="col-xs-12 col-md-6 position-absolute"
        style={{ fontSize: "30px", fontWeight: "bold" }}
      >
        <div className="col-xs-12 d-flex justify-content-end ms-5">
          Tech-driven IT services delivering secure, scalable, and on-demand
          solutions for digital transformation and business agility
        </div>
        <div className="col-xs-12 d-flex justify-content-middle ms-5">
          <Button href="/contactUs" className="button-common-style">
            Request Demo
          </Button>
        </div>
      </div>
